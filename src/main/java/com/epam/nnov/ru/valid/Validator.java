...
public class Validator {
    private static final String REGEX_NUM_OP_NUM = "(\\d+\\.?\\d*)(\\*?\\/?\\+?\\-?)(\\d+\\.?\\d*)";
    private static final String REGEX_NUM_IN_BRACKETS = "(\\(\\-?\\d+\\.?\\d*\\))";
    private static final String REGEX_BAD_NUM = "(\\d+\\.\\d+\\.)";
    private static final String WRONG_SYMB_COMBS = "(\\)\\d)|(\\)\\()|(\\d\\()|(\\([\\-|\\+|\\*|\\/]\\d+\\.?\\d*(?!\\)|\\.)\\D)";
    ...
    public boolean isMathExpressionValid(String mathExpression) throws UnsupportedSymbolsInMathExpression, BracketsMissmatchException {
        ...
        Matcher matchBadNum = Pattern.compile(REGEX_BAD_NUM).matcher(expression);
        Matcher wrongSymbCombs = Pattern.compile(WRONG_SYMB_COMBS).matcher(expression);
    ...
    private String collapseNegNum (String expression) {
        Matcher matchNumInBrackets = Pattern.compile(REGEX_NUM_IN_BRACKETS).matcher(expression);
        ...
        while (findNumInBrackets) {
            ...
            matchNumInBrackets = Pattern.compile(REGEX_NUM_IN_BRACKETS).matcher(expression);
    ...
    private String collapseNumOpNum (String expression) {
        Matcher matcherNumOpNum = Pattern.compile(REGEX_NUM_OP_NUM).matcher(expression);
        boolean findNumOpNum = matcherNumOpNum.find();
        while (findNumOpNum) {
                expression = new StringBuilder(expression)
                        .replace(matcherNumOpNum.start(1), matcherNumOpNum.end(3), "0").toString();
                matcherNumOpNum = Pattern.compile(REGEX_NUM_OP_NUM).matcher(expression);
